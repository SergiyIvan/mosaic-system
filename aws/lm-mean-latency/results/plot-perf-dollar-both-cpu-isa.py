...
if __name__ == "__main__":
    ...
    # ------------------ SUBPLOT 1: DEFAULT (ISA-BINDING) ------------------
    d_data = plot_data["default"]

    # Intel c7i
    ax1.bar(x - width, d_data["c7i"]["val"], yerr=d_data["c7i"]["err"], width=width,
            label='Intel (C7i)', edgecolor='black', alpha=0.85, hatch='', error_kw=err_kw)
    # AMD c7a
    ax1.bar(x, d_data["c7a"]["val"], yerr=d_data["c7a"]["err"], width=width,
            label='AMD (C7a)', edgecolor='black', alpha=0.85, hatch='//', error_kw=err_kw)
    # Graviton c7g
    ax1.bar(x + width, d_data["c7g"]["val"], yerr=d_data["c7g"]["err"], width=width,
    ...
    ax2.bar(x - width, o_data["c7i"]["val"], yerr=o_data["c7i"]["err"], width=width,
            label='Intel (C7i)', edgecolor='black', alpha=0.85, hatch='', error_kw=err_kw)
    ax2.bar(x, o_data["c7a"]["val"], yerr=o_data["c7a"]["err"], width=width,
            label='AMD (C7a)', edgecolor='black', alpha=0.85, hatch='//', error_kw=err_kw)
    ax2.bar(x + width, o_data["c7g"]["val"], yerr=o_data["c7g"]["err"], width=width,
            label='Graviton (C7g)', edgecolor='black', alpha=0.85, hatch='\\\\', error_kw=err_kw)
    ...
    fig.text(0.06, 0.5, "Requests/$ (Relative to Intel)", va='center', rotation='vertical')

    # Subplot Titles / Labels
    fig.text(0.15, 0.82, 'ISA-binding', ha='left', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))
    fig.text(0.15, 0.40, 'CPU-binding', ha='left', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))

    ax1.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    ax2.set_xticks(x)
    ax1.set_xticklabels(["" for _ in target_benchmarks], ha="right")
    ...
    # Move legend above the first plot
    ax1.legend(loc='upper center', bbox_to_anchor=(0.5, 1.25), ncol=3)

    fig.subplots_adjust(hspace=0.15)

    output_file = "motivation-perf-dollar-cpu-isa.pdf"
