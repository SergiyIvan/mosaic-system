...
def load_machine_data(machine_dir, hourly_cost):
    ...
    for flavor in FLAVORS:
        for bench in target_benchmarks:
            ...
            # Convert Requests Per Second to Requests Per Dollar
            req_per_dollar = (rps * 3600.0) / hourly_cost
            flavor_data[flavor].append(req_per_dollar)
...
if __name__ == "__main__":
    ...
    # Gather raw data for all configurations
    raw_cluster_data = {}
    for m in MACHINES:
        raw_cluster_data[m] = load_machine_data(m, COSTS_PER_HOUR[m])
    ...
    for m in MACHINES:
        ...
        # Plot Mosaic (Left position)
        ax.bar(x - width, normalized_cluster_data[m]["mo"], width=width, label=LABELS["mo"],
               color=styles["mo"]["color"], edgecolor='black', alpha=0.8,
        ...
        ax.bar(x, normalized_cluster_data[m]["na_de"], width=width, label=LABELS["na_de"],
               color=styles["na_de"]["color"], edgecolor='black', alpha=0.8,
        ...
        ax.bar(x + width, normalized_cluster_data[m]["na_na"], width=width, label=LABELS["na_na"],
               color=styles["na_na"]["color"], edgecolor='black', alpha=0.8,
               hatch=styles["na_na"]["hatch"])

        # Add a reference baseline horizontal line at y=1.0
        ax.axhline(1.0, color='red', linestyle='--', linewidth=1, zorder=1)
    ...
    # Shared Labels & Adjustments
    fig.text(0.04, 0.5, "Perf/$ (Relative to ISA-binding)", va='center', rotation='vertical', fontsize=15)

    ax3.set_xticks(x)
    ax3.set_xticklabels(target_benchmarks, rotation=25, ha="right")
    ...
    fig.subplots_adjust(hspace=0.2, top=0.90, bottom=0.12)

    output_file = "perf-dollar-machine-paper.pdf"
