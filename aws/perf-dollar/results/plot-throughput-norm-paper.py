...
def plot_machine_results(machine_id, machine_title, opt_config):
    ...
    ax.bar(x - 0.5*width, m_nd, yerr=s_nd, width=width, label='Default',
           edgecolor='black', alpha=0.75, hatch='..', error_kw=err_kw, capsize=4)

    # Native Native.
    m_nn, s_nn = norm(*stats["native_native"])
    ax.bar(x + 0.5*width, m_nn, yerr=s_nn, width=width, label='Optimized',
           edgecolor='black', alpha=0.75, hatch='', error_kw=err_kw, capsize=4)
    ...
    output_file = f"throughput-norm-{machine_id}-paper.pdf"
