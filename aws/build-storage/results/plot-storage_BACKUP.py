...
print(f"Discovered {len(unique_functions)} unique functions.")
...
# Sum all trampolines (native) and CPU ffmpeg across all 3 machines exactly ONCE
for m in MACHINES:
...
for bench in TARGET_BENCHMARKS:
    ...
    # 1. Mosaic (Wasm Guest)
    # Wasm size is uniform, taking from c7i
    ...
    # 3. Uber-Mode (Sum across all 3 architectures)
    uber_size = 0
    for m in MACHINES:
    ...
    # 5. ISA-Binding (Perf Winner)
    # Rule: If c7g won, use c7g. If c7a or c7i won, force c7i.
...
for mode, size_bytes in storage.items():
    ...
    print(f"{mode:<15} : {storage[mode]:.2f} GB")
...
bars = ax.bar(labels, values, color=colors, hatch=hatches, width=0.65, edgecolor='black', linewidth=1.2, alpha=0.85)

ax.set_ylabel("Storage Footprint (GBs)")
ax.grid(axis='y', linestyle='--', alpha=0.7)

ax.set_ylim(ymin=0, ymax=(max(values) + max(values) * 0.15))

# Add text labels on top of the bars
for bar in bars:
    yval = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2, yval + (max(values)*0.02),
            f'{yval:.1f}', ha='center', va='bottom', fontsize=14, fontweight='bold')
