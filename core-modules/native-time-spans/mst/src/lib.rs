...
pub fn run(size: usize, m: usize) -> u32 {
    ...
    let start_mst = std::time::Instant::now();
    let edges_in_tree = mst(&edges, &mut mst_edges_out, size);
    println!("*** Span: LibCode_MST | DurationUs: {}", start_mst.elapsed().as_micros());
...
pub fn proxy_handler(input_json: &str) -> String {
    ...
    let result = if edges_in_tree == expected_edges as u32 {
        format!("Success: MST constructed with {} edges.", edges_in_tree)
    } else {
        format!("Error: MST constructed with {}/{} edges.", edges_in_tree, expected_edges)
    };
