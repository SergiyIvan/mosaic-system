...
pub fn run(model_url: &str, image_url: &str, labels_url: &str, _model_file_size: usize, image_file_size: usize, _labels_file_size: usize) -> String {
    ...
    if !std::path::Path::new(model_path).exists() {
        let start_dl_model = std::time::Instant::now();
        download_to_file(model_url, model_path);
        println!("*** Span: LibCode_DownloadModel | DurationUs: {}", start_dl_model.elapsed().as_micros());
    }
    ...
    if !std::path::Path::new(labels_path).exists() {
        let start_dl_labels = std::time::Instant::now();
        download_to_file(labels_url, labels_path);
        println!("*** Span: LibCode_DownloadLabels | DurationUs: {}", start_dl_labels.elapsed().as_micros());
    }
    ...
    let start_dl_img = std::time::Instant::now();
    let img_size = download(image_url, &mut img_buf);
    println!("*** Span: LibCode_DownloadImage | DurationUs: {}", start_dl_img.elapsed().as_micros());
    ...
    println!("*** Span: LibCode_Infer | DurationUs: {}", start_infer.elapsed().as_micros());
...
pub fn proxy_handler(input_json: &str) -> String {
    ...
    let result = if class_name.is_empty() {
