...
pub extern "C" fn run() -> u32 {
    ...
    // Command 1: Extract GIF (Duration: 10 seconds).
    let cmd_gif = format!(
        "/tmp/benchmark/ffmpeg -y -i {} -t 5 -vf 'fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse' -loop 0 /tmp/benchmark/processed.gif > /dev/null 2>&1",
        video_path
    );

    // Command 2: Watermark (Duration: 20 seconds).
    let cmd_watermark = format!(
        "/tmp/benchmark/ffmpeg -y -i {} -i {} -t 5 -filter_complex 'overlay=main_w/2-overlay_w/2:main_h/2-overlay_h/2' /tmp/benchmark/watermarked.mp4 > /dev/null 2>&1",
